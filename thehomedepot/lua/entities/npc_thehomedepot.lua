...
local NextbotSounds = {
	...
	["HighJump"] = "thehomedepot/jumpsound.mp3",
	["Jump"] = "thehomedepot/jumpsound.mp3",
	["ChaseSound"] = "thehomedepot/chasesound.mp3",
	["tauntSounds"] = {
 "thehomedepot/attacksound.mp3",
}
...
local npc_thehomedepot_acquire_distance = CreateConVar("npc_thehomedepot_acquire_distance", 2500, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which thehomedepot will chase a target.")

local npc_thehomedepot_spawn_protect = CreateConVar("npc_thehomedepot_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, thehomedepot will not target players or hide within 200 units of a spawn point.")

local npc_thehomedepot_attack_distance = CreateConVar("npc_thehomedepot_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of thehomedepot's attack.")

local npc_thehomedepot_attack_interval = CreateConVar("npc_thehomedepot_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between thehomedepot's attacks.")

local npc_thehomedepot_attack_force = CreateConVar("npc_thehomedepot_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of thehomedepot's attack. Higher values throw things farther.")

local npc_thehomedepot_smash_props = CreateConVar("npc_thehomedepot_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, thehomedepot will punch through any props placed in their way.")

local npc_thehomedepot_hiding_scan_interval = CreateConVar("npc_thehomedepot_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "thehomedepot will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant thehomedepots are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local npc_thehomedepot_hiding_repath_interval = CreateConVar("npc_thehomedepot_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to thehomedepot's hiding spot will be redetermined every X seconds.")

local npc_thehomedepot_chase_repath_interval = CreateConVar("npc_thehomedepot_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of thehomedepot's target will be redetermined every X seconds.")

local npc_thehomedepot_expensive_scan_interval = CreateConVar("npc_thehomedepot_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local npc_thehomedepot_force_download = CreateConVar("npc_thehomedepot_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download thehomedepot resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear thehomedepot!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

util.AddNetworkString("thehomedepot_nag")
util.AddNetworkString("thehomedepot_nagresponse")
util.AddNetworkString("thehomedepot_navgen")
...
local VECTOR_thehomedepot_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_thehomedepot_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_thehomedepot: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other thehomedepots, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC() and
	        ent:Health() > 0 and
	        class ~= "npc_thehomedepot" and
	        not class:find("bullseye"))
end

--HACK!!! Because this is an issue a lot of people keep asking me about.
hook.Add("PlayerSpawnedNPC", "thehomedepotMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_thehomedepot") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.thehomedepot_HasBeenNagged) then return end
	ply.thehomedepot_HasBeenNagged = true

	-- Try to explain why thehomedepot isn't working.
	net.Start("thehomedepot_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("npc_thehomedepot: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_thehomedepot: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("thehomedepot_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.thehomedepot_HasBeenNagged = false
		return
	end
	...
	--for _, thehomedepot in pairs(ents.FindByClass("npc_thehomedepot")) do
	--	thehomedepot:Remove()
	--end

	print("npc_thehomedepot: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "thehomedepotNavGen", navEndGenerate)
	else
		print("npc_thehomedepot: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_thehomedepot_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `npc_thehomedepot_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (npc_thehomedepot_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_thehomedepot_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. thehomedepot means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * npc_thehomedepot_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * npc_thehomedepot_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not npc_thehomedepot_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_thehomedepot_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_thehomedepot_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > npc_thehomedepot_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > npc_thehomedepot_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(npc_thehomedepot_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > npc_thehomedepot_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= npc_thehomedepot_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= npc_thehomedepot_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= npc_thehomedepot_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= npc_thehomedepot_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local thehomedepotsLastFrame = 0
hook.Add("Tick", "thehomedepotBenchmark", function()
	local now = SysTime()
	local numthehomedepots = #ents.FindByClass("npc_thehomedepot")

	if (timeAll ~= 0) then

		local thehomedepotTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    thehomedepotTime / thehomedepotsLastFrame, thehomedepotTime, frameTime, (thehomedepotTime / frameTime) * 100, thehomedepotsLastFrame, numThink, (numThink / thehomedepotsLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	thehomedepotsLastFrame = numthehomedepots
end)]]
...
local function DevPrint(devLevel, msg)
	if (developer:GetInt() >= devLevel) then
		print("npc_thehomedepot: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a thehomedepot.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_thehomedepot_music_volume = CreateConVar("npc_thehomedepot_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by thehomedepot. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another thehomedepot comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, thehomedepots do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_thehomedepot_PANIC_COUNT thehomedepots are this close, or an equivalent score.
local MUSIC_thehomedepot_PANIC_COUNT = 8 -- That's a lot of thehomedepot.

local MUSIC_thehomedepot_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_thehomedepot_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("npc_thehomedepot") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("thehomedepotPanicMusicUpdate")
	...
	if (npc_thehomedepot_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		panicMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "npc_thehomedepot") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_thehomedepot_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear thehomedepot taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * npc_thehomedepot_music_volume:GetFloat() * 5)
...
local function startTimer()
	if (not timer.Exists("thehomedepotPanicMusicUpdate")) then
		timer.Create("thehomedepotPanicMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local thehomedepotMaterial = Material("thehomedepot/image.png")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(thehomedepotMaterial)
...
hook.Add("OnEntityCreated", "thehomedepotInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_thehomedepot") then return end

	local thehomedepotEntTable = scripted_ents.GetStored("npc_thehomedepot")

	table.Merge(ent, thehomedepotEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("thehomedepot_removed", thehomedepotDeregister)
end)

hook.Add("NetworkEntityCreated", "thehomedepotNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_thehomedepot") then return end
...
surface.CreateFont("thehomedepotHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("thehomedepotHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("thehomedepot is studying this map.", "thehomedepotHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "thehomedepotHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "thehomedepotHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "thehomedepotHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "thehomedepotHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("thehomedepot_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "thehomedepotNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. thehomedepot doesn't even know where to start with this map.
...
local function navGenerate()
	net.Start("thehomedepot_nagresponse")
		net.WriteBit(true)
	net.SendToServer()
end
...
local function navWarning()
	Derma_Query([[It will take a while for thehomedepot to figure this map out.
...
-- Lazy
net.Receive("thehomedepot_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! thehomedepot doesn't know this map.
Would you like thehomedepot to learn it?]],
		            "This map is currently not thehomedepot-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! thehomedepot doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to thehomedepot.]],
		            "This map is currently not thehomedepot-compatible!",
		            "Ok", nagAgain,
...
--
list.Set("NPC", "npc_thehomedepot", {
	Name = "The Home Depot",
	Class = "npc_thehomedepot",
	Category = "Nextbot",
	AdminOnly = True
})
